...
class PyRace2D:
    ...
    def action(self, action):
        # Enhanced action space with more fine-grained control
        if action == 0:  # Small acceleration
            self.car.speed += 1
        elif action == 1:  # Large acceleration
            self.car.speed += 3
        elif action == 2:  # Small left turn
            self.car.angle += 3
        elif action == 3:  # Large left turn
            self.car.angle += 6
        elif action == 4:  # Small right turn
            self.car.angle -= 3
        elif action == 5:  # Large right turn
            self.car.angle -= 6
        elif action == 6:  # Brake
            self.car.speed = max(1, self.car.speed - 2)
        ...
        # Increased number of sensors - every 30 degrees instead of 45
        for d in range(-90, 135, 30):
    ...
    def evaluate(self):
        ...
        # Add time-based reward to encourage faster completion
        if self.car.check_flag:
            self.car.check_flag = False
            # Reward for checkpoint - higher for faster times
            reward = 500 - self.car.time_spent/10
        ...
        if not self.car.is_alive: # crash
            ...
            completion_time_bonus = max(0, 2000 - self.car.time_spent)
            reward = 10000 + completion_time_bonus
        
        # Add small reward for maintaining speed
        if self.car.is_alive and not self.car.goal:
    ...
    def observe(self):
        # Enhanced observation with 9 radar readings + car state
        radars = self.car.radars
        ret = [0] * 9  # Now 9 radar sensors
        
        for i, r in enumerate(radars):
            ret[i] = int(r[1] / 20)
            
        # Add car state: normalized speed
        speed_normalized = self.car.speed / 10.0
        
        # Add car angle (normalized between -1 and 1 using sin)
        angle_normalized = math.sin(math.radians(self.car.angle))
        
        # Add distance to next checkpoint (normalized)
        if len(check_point) > 0:
            p = check_point[self.car.current_check % len(check_point)]
            checkpoint_distance = get_distance(p, self.car.center)
            checkpoint_distance_normalized = min(1.0, checkpoint_distance / 1000.0)
        else:
            checkpoint_distance_normalized = 0
            
        # Add these additional observations to the radar readings
        ret.extend([speed_normalized, angle_normalized, checkpoint_distance_normalized])
